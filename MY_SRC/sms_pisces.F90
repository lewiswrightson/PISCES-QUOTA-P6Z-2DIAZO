MODULE sms_pisces   
  ...
  ! LOGICAL  ::  ln_tricho         !: switch between tricho and croco parameter
   ...
   REAL(wp), ALLOCATABLE, SAVE, DIMENSION(:,:,:) ::  ecroc, ecrocm  !: PAR for diazotroph2
   REAL(wp), ALLOCATABLE, SAVE, DIMENSION(:,:,:) ::  vnfmax_cr   !: Maximum nutrient uptake rate diazotroph2
   ...
   REAL(wp), ALLOCATABLE, SAVE, DIMENSION(:,:,:) ::   sizecr     !: size of diazotroph2
   REAL(wp), ALLOCATABLE, SAVE, DIMENSION(:,:,:) ::   sizecra    !: size of diazotroph2, after
...
CONTAINS
   ...
   INTEGER FUNCTION sms_pisces_alloc()
      ...
      IF( ln_p6z ) THEN
         !
         ALLOCATE(ediaz(jpi,jpj,jpk)   , ediazm(jpi,jpj,jpk),      &
           &      ecroc(jpi,jpj,jpk)   , ecrocm(jpi,jpj,jpk),      STAT=ierr(11) )
         !* size of diazotroph cells
         ALLOCATE(  sizedz(jpi,jpj,jpk), sizedza(jpi,jpj,jpk),     &
           &        sizecr(jpi,jpj,jpk), sizecra(jpi,jpj,jpk),     STAT=ierr(12) )
         !* Maximum nutrient uptake by diazotroph
         ALLOCATE( vnfmax_dz(jpi,jpj,jpk), vnfmax_cr(jpi,jpj,jpk), STAT=ierr(13) )
      ENDIF
      !
