   ...
   SUBROUTINE p4z_rst( kt, cdrw )
      ...
      IF( TRIM(cdrw) == 'READ' ) THEN
        ...
        ENDIF
         ...
         ! PISCES-QUOTA Explict Diazotrophy
         IF( ln_p6z ) THEN
            ! Read the size of the different phytoplankton groups
            ! If not in the restart file, they are set to 1
            IF( iom_varid( numrtr, 'sizep', ldstop = .FALSE. ) > 0 ) THEN
            ...
            ELSE
               vnfmax_dz(:,:,:) = 1.
            ENDIF
            IF( iom_varid( numrtr, 'sizecr', ldstop = .FALSE. ) > 0 ) THEN
               CALL iom_get( numrtr, jpdom_autoglo, 'sizecr' , sizecr(:,:,:)  )
            ELSE
               sizecr(:,:,:) = 1.
            ENDIF
            IF( iom_varid( numrtr, 'vnfmax_cr', ldstop = .FALSE. ) > 0 ) THEN
               CALL iom_get( numrtr, jpdom_autoglo, 'vnfmax_cr' , vnfmax_cr(:,:,:)  )
            ELSE
               vnfmax_cr(:,:,:) = 1.
            ENDIF
        ENDIF
      ...
      ELSEIF( TRIM(cdrw) == 'WRITE' ) THEN
         ...
         IF( ln_p6z ) THEN
            ...
            CALL iom_rstput( kt, nitrst, numrtw, 'sizecr', sizecr(:,:,:) )
         ENDIF
      ENDIF
      !
