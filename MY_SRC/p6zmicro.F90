MODULE p6zmicro
   ...
   REAL(wp), PUBLIC ::  xprefcr     !: Microzoo preference for diazotroph2
   REAL(wp), PUBLIC ::  xthreshcr   !: Diazotroph2 feeding threshold for microzooplankton
...
CONTAINS
   ...
   SUBROUTINE p6z_micro( kt, knt )
      ...
      REAL(wp) :: zcompacr, ztmp7, zgrazcrc, zgrazcrn, zgrazcrp, zgrazcrf, zdiffcrn, zdiffcrp, zdiffcrd, zproport4
      REAL(wp) :: zdiffcrdz
      REAL(wp), DIMENSION(jpi,jpj,jpk) :: zgrazing, zfezoo, znitzoo, micrograz_dzc, micrograz_crc
      ...
      DO jk = 1, jpkm1
         DO jj = 1, jpj
            DO ji = 1, jpi
               ...
               zproport4 = sizecr(ji,jj,jk)**(-0.54)
            ...
            !  zproport2  = min(1.0, exp(-1.1 * MAX(0., ( sizen(ji,jj,jk) - 1.8 ))**0.8 ))
             ! zproport3  = min(1.0, exp(-1.1 * MAX(0., ( sizedz(ji,jj,jk) - 1.8 ))**0.8 ))
               ...
               zcompacr  = zproport4 * MAX( ( trb(ji,jj,jk,jpccr) - xthreshcr ), 0.e0 )
               ...
               zdiffcrp = exp( -ABS(log(0.5 * sizep(ji,jj,jk) / (3.0 * sizecr(ji,jj,jk) + rtrn )) )**2 / zsigma**2 )
               ...
               &       + zdiffcrn * zcompacr) / ( 1.0 + zdiffdn + zdiffpn + zdiffdzn + zdiffcrn)
               ztmp2 = xprefp * zcompapi * ( zcompapi + zdiffpn * zcompaph + zdiffdp * zcompadi + zdiffdzp * zcompadz    &
               &       + zdiffcrp * zcompacr) / ( 1.0 + zdiffpn + zdiffdp + zdiffdzp + zdiffcrp )
               ztmp3 = xprefc * zcompapoc**2
               ztmp4 = xprefd * zcompadi * ( zdiffdp * zcompapi + zdiffdn * zcompaph + zcompadi + zdiffdzd * zcompadz    &
               &       + zdiffcrd * zcompacr ) / ( 1.0 + zdiffdn + zdiffdp + zdiffdzd + zdiffcrd )
               ztmp5 = xprefz * zcompaz**2
               ztmp6 = xprefdz * zcompadz * ( zcompadz + zdiffdzn * zcompaph + zdiffdzp * zcompapi + zdiffdzd * zcompadi  &
               &       + zdiffcrdz * zcompacr) / ( 1.0 + zdiffdzn + zdiffdzp + zdiffdzd + zdiffcrdz )
               ztmp7 = xprefcr * zcompacr * ( zcompacr + zdiffcrn * zcompaph + zdiffcrp * zcompapi + zdiffcrd * zcompadi  &
               &       + zdiffcrdz * zcompadz) / ( 1.0 + zdiffcrn + zdiffcrp + zdiffcrd + zdiffcrdz )
               ...
               zgraztotc = zgraznc + zgrazpoc + zgrazdc + zgrazz + zgrazpc + zgrazdzc + zgrazcrc
               zgraztotn = zgraznn + zgrazpn + zgrazpon + zgrazdn + zgrazdzn + zgrazcrn + zgrazz * no3rat3
               zgraztotp = zgraznp + zgrazpp + zgrazpop + zgrazdp + zgrazdzp + zgrazcrp + zgrazz * po4rat3
               zgraztotf = zgraznf + zgrazpf + zgrazpof + zgrazdf + zgrazdzf + zgrazcrf + zgrazz * ferat3
               !
               ! Grazing by microzooplankton
               zgrazing(ji,jj,jk) = zgraztotc
               micrograz_dzc(ji,jj,jk) = zgrazdzc
               micrograz_crc(ji,jj,jk) = zgrazcrc
      ...
      IF( lk_iomput ) THEN
         IF( knt == nrdttrc ) THEN
            ...
            ENDIF
            IF( iom_use( "NITZOO" ) ) THEN
               zw3d(:,:,:) = znitzoo(:,:,:) * 1.e+3 * rfact2r * tmask(:,:,:)   !
               CALL iom_put( "NITZOO", zw3d )
            ENDIF
            IF( iom_use( "MICRO_GRAZ_DZC" ) ) THEN
               zw3d(:,:,:) = micrograz_dzc(:,:,:) * 1.e+3 * rfact2r * tmask(:,:,:)  ! Total grazing of diazo by zooplankton
               CALL iom_put( "MICRO_GRAZ_DZC", zw3d )
            ENDIF
            IF( iom_use( "MICRO_GRAZ_CRC" ) ) THEN
               zw3d(:,:,:) = micrograz_crc(:,:,:) * 1.e+3 * rfact2r * tmask(:,:,:)  ! Total grazing of diazo by zooplankton
               CALL iom_put( "MICRO_GRAZ_CRC", zw3d )
            ENDIF
            DEALLOCATE( zw3d )
         ENDIF
      ENDIF
   ...
   SUBROUTINE p6z_micro_init
      ...
      NAMELIST/namp6zzoo/ part, grazrat, bmetexc, resrat, mzrat, xprefc, xprefn, &
         ...
         &                xsigma, xsigmadel, xprefdz, xthreshdz, xprefcr, xthreshcr  
      !!----------------------------------------------------------------------
      !
...
902   IF( ios >  0 ) CALL ctl_nam ( ios , 'namp6zzoo in configuration namelist' )
      ...
      IF(lwp) THEN                         ! control print
         ...
         WRITE(numout,*) '    microzoo preference for diazotroph2               xprefcr     =', xprefcr
         WRITE(numout,*) '    diazotroph2 feeding threshold for microzoo         xthreshcr   =', xthreshcr
      ENDIF
      !
