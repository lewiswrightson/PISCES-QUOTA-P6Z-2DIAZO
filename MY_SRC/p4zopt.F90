...
CONTAINS
   ...
   SUBROUTINE p4z_opt( kt, knt )
      ...
      REAL(wp), ALLOCATABLE, DIMENSION(:,:  ) :: zetmp5, zetmp6, zetmp7
      ...
      IF( ln_p6z ) ALLOCATE( zetmp6(jpi,jpj), zetmp7(jpi,jpj) )
      ...
      IF( ln_p6z )   zchl3d(:,:,:) = zchl3d(:,:,:) + trb(:,:,:,jppch) + trb(:,:,:,jpchd) + trb(:,:,:,jpchc)
      !
      ...
      IF( l_trcdm2dc ) THEN                     !  diurnal cycle
         ...
         IF( ln_p6z ) THEN
            DO jk = 1, nksrp
              ...
              ecroc  (:,:,jk) =  1.85 * ze1(:,:,jk) + 0.69 * ze2(:,:,jk) + 0.46 * ze3(:,:,jk)
            END DO
         ENDIF
         ! SW over the ice free zone of the grid cell. This assumes that
         ! SW is zero below sea ice which is a very crude assumption that is 
      ...
      ELSE ! no diurnal cycle
         ...
         IF( ln_p6z ) THEN
            DO jk = 1, nksrp
              ...
              ecroc(:,:,jk) =  1.85 * ze1(:,:,jk) + 0.69 * ze2(:,:,jk) + 0.46 * ze3(:,:,jk) ! Explicit Diazotroph2 (PISCES QUOTA)
            END DO
         ENDIF
         etot_ndcy(:,:,:) =  etot(:,:,:) 
      ENDIF
...
!
      IF( ln_p6z ) THEN
         ! Explicit diazotrophs when using PISCES-QUOTA
         zetmp6 (:,:) = 0.e0
         zetmp7 (:,:) = 0.e0
         DO jk = 1, nksrp
            DO jj = 1, jpj
               DO ji = 1, jpi
                  IF( gdepw_n(ji,jj,jk+1) <= MIN(hmld(ji,jj), heup_01(ji,jj)) ) THEN
                     ...
                     zetmp7(ji,jj)  = zetmp7 (ji,jj) + ecroc(ji,jj,jk) * e3t_n(ji,jj,jk)
                  ENDIF
               END DO
            END DO
         END DO
            ...
            DO jj = 1, jpj
               DO ji = 1, jpi
                  IF( gdepw_n(ji,jj,jk+1) <= hmld(ji,jj) ) THEN
                     ...
                     ecrocm(ji,jj,jk) = zetmp7(ji,jj) * z1_dep
                  ENDIF
               END DO
            END DO
         END DO
      ...
      IF( lk_iomput ) THEN
        IF( knt == nrdttrc ) THEN
           ...
           IF( iom_use( "Hmld"  ) ) CALL iom_put( "Hmld" , hmld(:,:  ) * tmask(:,:,1) )  ! mixed layer deptht
        ENDIF
      ENDIF
      !
      IF( ln_p5z    )   DEALLOCATE( zetmp5 )
      IF( ln_p6z    )   DEALLOCATE( zetmp5, zetmp6, zetmp7 )
