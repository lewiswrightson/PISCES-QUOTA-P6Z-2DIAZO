...
CONTAINS
   ...
   SUBROUTINE p4z_poc( kt, knt )
      ...
      REAL(wp), DIMENSION(jpi,jpj,jpk)   :: zremipoc, zremigoc, zorem3, ztremint, zfolimi, znremin
        ...
        DO jk = 1, jpkm1
           DO jj = 1, jpj
              DO ji = 1, jpi
                 ...
                 tra(ji,jj,jk,jpfer) = tra(ji,jj,jk,jpfer) + zofer 
                 zfolimi(ji,jj,jk)   = zfolimi(ji,jj,jk) + zofer
                 znremin(ji,jj,jk)   = znremin(ji,jj,jk) + zopon
              END DO
           END DO
        END DO
     ENDIF
     ...
     IF( lk_iomput ) THEN
        ...
        IF(ln_p6z) THEN
                    CALL iom_put( "REMINN" , znremin(:,:,:) * tmask(:,:,:)* 1e+9 * zrfact2 )  !Remineralisation rate of N 
        ENDIF
        ENDIF
     ENDIF
