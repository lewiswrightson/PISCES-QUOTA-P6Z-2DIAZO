...
CONTAINS
   ...
   SUBROUTINE p4z_ini
      ...
      DO jn = 1, jptra
        ...
        IF( cltra == 'CCR'      )   jpccr = jn      !: Diazotroph2 C biomass
        IF( cltra == 'FEC'      )   jpfec = jn      !: Diazotroph2 Fe biomass
        IF( cltra == 'CHC'      )   jpchc = jn      !: Diazotroph2 Chl biomass
        IF( cltra == 'NCR'      )   jpncr = jn      !: Diazotroph2 N biomass
        IF( cltra == 'PCR'      )   jppcr = jn      !: Diazotroph2 P biomass 
      END DO
      ...
      IF( .NOT.ln_rsttr ) THEN  
         ...
         IF( ln_p6z ) THEN
            ...
            trn(:,:,:,jppdz) = bioma0
            trn(:,:,:,jpccr) = bioma0
            trn(:,:,:,jpfec) = bioma0 * 5.e-6
            trn(:,:,:,jpchc) = bioma0 * 12. / 55.
            trn(:,:,:,jpncr) = bioma0
            trn(:,:,:,jppcr) = bioma0
         ENDIF
